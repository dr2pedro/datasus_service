...
import { deserializeSIHCriteria } from "./deserializeSIHCriterias.js";


/** The main job, the guy that do the thing!. This is tested in parent classes. */
process.on('message', async (msg: JobMessage<AIH | SP>) => {
    if(msg.output === 'file') {
    ...
    if(msg.criterias) {
        await dbc.forEachRecords(async(record: AIH | SP) => {
            let criterias: Criteria<AIH>[] = [];
            
            // Dealing with the serialization.
            msg.criterias?.map((criteria: Criteria<AIH | SP>) => {
                criteria = deserializeSIHCriteria(criteria)!
